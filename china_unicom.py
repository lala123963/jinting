...
3. 环境变量说明 MULTI_PHONE_NUM(必需) UNICOM_LOTTER(选填) 自行新建环境变量添加
    MULTI_PHONE_NUM 为你的手机号 phone1&phone2
...
if __name__ == "__main__":
    """读取环境变量"""
    temp = get_environ("MULTI_PHONE_NUM")
    unicom_lotter = environ.get("UNICOM_LOTTER") if environ.get("UNICOM_LOTTER") else True

    multi_phone_num = []
    if temp == "" :
        print("未填写 TELECOM_USERPASS 变量 退出")
        exit(0)
    if len(temp.split('\n')) > 1:
        print("手机号密码使用换行隔开")
        multi_phone_num = temp.split('\n')
